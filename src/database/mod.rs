...
pub trait NickDatabase: Send + Sync {
    ...
    async fn add_nick(&mut self, nick: &str, password: &str, user: &str, registration_time: SystemTime, email: Option<&str>, url: Option<&str>, vhost: Option<&str>, noaccess: bool, noop: bool, showmail: bool) -> Result<(), Box<dyn Error + Send + Sync>>;
    async fn get_nick_info(&self, nick: &str) -> Result<Option<(String, SystemTime, Option<String>, Option<String>, Option<String>, bool, bool, bool)>, Box<dyn Error + Send + Sync>>;
    ...
    async fn update_nick_info(&mut self, nick: &str, user: Option<&str>, registration_time: Option<SystemTime>, email: Option<&str>, url: Option<&str>, vhost: Option<&str>, noaccess: Option<bool>, noop: Option<bool>, showmail: Option<bool>) -> Result<(), Box<dyn Error + Send + Sync>>;
